...
/* ---------------------------------------------------------------------
   고객문의 폼 — 서버로 보내고 결과를 그 자리에서 알려 준다.
   화면을 새로 고치지 않으므로 입력한 내용이 사라지지 않는다.
   --------------------------------------------------------------------- */
(function () {
  ...
  function show(text, ok) {
    ...
    msg.className = 'cform__msg' + (ok ? ' is-ok' : ' is-error');
    msg.hidden = false;
  }
  ...
  form.addEventListener('submit', function (e) {
    ...
    // 브라우저 기본 검사를 먼저 통과시킨다(어느 칸이 비었는지 표시해 준다).
    if (!form.checkValidity()) {
      form.reportValidity();
      return;
    }

    if (btn) { btn.disabled = true; if (btnText) btnText.textContent = '보내는 중…'; }
    ...
    fetch(form.action, {
      ...
      .then(function (r) {
        // 다음 전송에 쓸 보안 토큰으로 갈아끼운다
        if (r.body && r.body.data && r.body.data.csrf_name) {
          var field = form.querySelector('input[name="' + r.body.data.csrf_name + '"]');
          if (field) field.value = r.body.data.csrf_hash;
        }
        if (r.body && r.body.status) {
          show(r.body.message, true);
          form.reset();
        } else {
          show((r.body && r.body.message) || '접수에 실패했습니다. 잠시 후 다시 시도해 주세요.', false);
        }
      })
      .catch(function () {
        show('네트워크 오류로 접수하지 못했습니다. 잠시 후 다시 시도해 주세요.', false);
      })
      .then(function () {
        if (btn) { btn.disabled = false; if (btnText) btnText.textContent = original; }
      });
  });
})();
